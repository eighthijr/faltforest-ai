...
async function callGenerateEndpoint(input: GenerateCopyInput): Promise<string> {
...
export async function generateCopyOnce(input: GenerateCopyInput): Promise<string> {
  ...
  const generatedCopy = await callGenerateEndpoint(input);
...
export async function generateRevisionCopy(input: GenerateCopyInput): Promise<string> {
  const generatedCopy = await callGenerateEndpoint(input);

  const { error } = await supabase.rpc('save_workspace_generated_copy', {
    p_project_id: input.projectId,
    p_generated_html: generatedCopy,
  });
